...
import { z } from 'zod';
import { X, Upload, File, AlertCircle, Building2 } from 'lucide-react';
import {
  gedService,
  type GedPasta,
  type GedCategoria,
...
// Variante do GedUploadModal para escopo EMPRESA. Usa o endpoint
// POST /api/v1/ged/documentos (sem obraId) e só lista pastas/categorias de
// escopo EMPRESA. O fluxo de escolha de obra antigo (EscolherObraModal + modal
// padrão) era gambiarra — este componente cobre o caminho arquiteturalmente
// correto.
...
interface GedUploadEmpresaModalProps {
  pastas: GedPasta[];
...
export function GedUploadEmpresaModal({
  pastas,
  categorias,
  onClose,
  onSuccess,
}: GedUploadEmpresaModalProps) {
  ...
  const onSubmit = async (formData: UploadFormData) => {
    ...
    try {
      ...
      await gedService.uploadEmpresa(fd, (pct) => setProgresso(pct));
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              ...
              <input
                {...register('titulo')}
                style={inputStyle}
                placeholder="Ex: Política de Qualidade Corporativa"
              />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <div>
                <label style={labelStyle}>Pasta (empresa) *</label>
                ...
                {pastas.length === 0 && (
                  <p style={{ ...errorStyle, color: 'var(--warn)' }}>
                    Nenhuma pasta corporativa cadastrada.
                  </p>
                )}
              </div>
          ...
          >
            ...
            <button
              type="submit"
              disabled={uploading || pastas.length === 0}
              style={{
                background: 'var(--accent)',
                border: 'none',
                color: '#000',
                borderRadius: '8px',
                padding: '9px 20px',
                cursor: uploading || pastas.length === 0 ? 'not-allowed' : 'pointer',
                fontSize: '14px',
                fontWeight: 600,
                opacity: uploading || pastas.length === 0 ? 0.7 : 1,
                display: 'flex',
                alignItems: 'center',
                gap: '8px',
              }}
